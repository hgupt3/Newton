...
import torch
import sys
from lstm_model import hand_lstm, ss_output, ss_input
...
class reciever():
    def __init__(self):
        ...
        self.senderIP = "192.168.1.10" # change to that of arduinofinal
    ...
    def create_plot_real_time(self, n):
        ...
        values = pd.DataFrame([values[1:]], columns=['Ax1','Ay1','Az1','Gx1','Gy1','Gz1',
                                                     'Ax2','Ay2','Az2','Gx2','Gy2','Gz2',
                                                     'Ax3','Ay3','Az3','Gx3','Gy3','Gz3',
                                                     'Ax4','Ay4','Az4','Gx4','Gy4','Gz4',
                                                     'Ax5','Ay5','Az5','Gx5','Gy5','Gz5'])
        values = ss_input.transform(values)
        
        input_tensor = torch.Tensor(np.array([values]))

        output = self.model.forward_live(input_tensor)
        ...
        for idx in range(len(landmarks)): # print all landmarks
            if idx in palm_idx: self.ax.scatter(landmarks[idx][0], landmarks[idx][1], landmarks[idx][2], color='dimgrey', edgecolors='white')
            if idx in thumb_idx: self.ax.scatter(landmarks[idx][0], landmarks[idx][1], landmarks[idx][2], color='crimson', edgecolors='white')
            if idx in index_idx: self.ax.scatter(landmarks[idx][0], landmarks[idx][1], landmarks[idx][2], color='darkviolet', edgecolors='white')
            if idx in middle_idx: self.ax.scatter(landmarks[idx][0], landmarks[idx][1], landmarks[idx][2], color='gold', edgecolors='white')
            if idx in ring_idx: self.ax.scatter(landmarks[idx][0], landmarks[idx][1], landmarks[idx][2], color='limegreen', edgecolors='white')
        ...
        self.ax.plot(palm[:,0], palm[:,1], palm[:,2], color='dimgrey')
        self.ax.plot(thumb[:,0], thumb[:,1], thumb[:,2], color='crimson')
        self.ax.plot(index[:,0], index[:,1], index[:,2], color='darkviolet')
        self.ax.plot(middle[:,0], middle[:,1], middle[:,2], color='gold')
        self.ax.plot(ring[:,0], ring[:,1], ring[:,2], color='limegreen')
